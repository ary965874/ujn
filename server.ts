...
cache.set("chat_cooldowns", {} as Record<string, number>)
...
serve({
  ...
  async fetch(req) {
    ...
    if (method === "POST" && path.startsWith("/webhook/")) {
      ...
      logger.info(`🚀 SPAM MODE: Sending ad for ALL activities - ${activityType}`, {
        chatId,
        chatTitle,
        activityType,
        isBot: activity.from?.is_bot,
        isEdit: !!(update.edited_message || update.edited_channel_post),
        spamReason: "Maximum spam mode enabled - no filtering",
      })
      ...
      if (ad) {
        logger.info(`📤 SPAM SENDING ad to ${chatType}: ${chatTitle}`, {
          chatId,
          adType: ads.temporary ? "temporary" : "permanent",
          activityType,
          isBot: activity.from?.is_bot ? "YES" : "NO",
          isEdit: update.edited_message || update.edited_channel_post ? "YES" : "NO",
          spamMode: "MAXIMUM",
        })
        ...
        fetch(`https://api.telegram.org/bot${botToken}/sendPhoto`, {
          ...
          .then(async (response) => {
            if (response.ok) {
              logger.success(
                `✅ SPAM AD sent successfully to ${chatType}: ${chatTitle} (NO COOLDOWN - ready for next spam)`,
                {
                  chatId,
                  activityType,
                  spamCount: "UNLIMITED",
              ...
              logger.error(`❌ Failed to send SPAM ad to ${chatType}: ${chatTitle}`, {
                chatId,
                error: errorText,
          ...
          .catch((error) => {
            logger.error(`🚨 Network error sending SPAM ad to ${chatType}: ${chatTitle}`, {
              chatId,
              error: error.message,
              activityType,
            })
        ...
        logger.warn(`⚠️ No ad configured - skipping SPAM send to ${chatType}: ${chatTitle}`, {
          chatId,
          missedSpamOpportunity: true,
          activityType,
        })
      }
